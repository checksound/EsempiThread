...
/**
 * This demo program uses a thread to compute an image "in the background". As
 * rows of pixels in the image are computed, they are copied to the screen. (The
 * image is a small piece of the famous Mandelbrot set, which is used just
 ...
 * button. A separate thread is created and is run at a lower priority, which
 * will make sure that the GUI thread will get a chance to run to repaint the
 * display as necessary. All changes to the GUI from the animation thread are
 * made by calling Platform.runLater().
 */
public class BackgroundComputationDemo extends Application {
	 ...
	 * running, it sets the signaling variable, running, to true and creates and
	 * starts a new thread. Note that the thread is responsible for changing the
	 * text on the button. Note that the priority of the thread is set to be one
	 * less than the priority of the thread that calls this method, that is of the
	 * JavaFX application thread. This means that the application thread is run in
	 * preference to the computation thread. When there is an event to be handled,
	 * such as updating the display or reacting to a button click, the
	 * event-handling thread should wake up immediately to handle the event.
	 */
