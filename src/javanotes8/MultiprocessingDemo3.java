...
/**
 * This demo program divides up a large computation into a fairly large number
 * of smaller tasks. The computation is to compute an image, and each task
 * computes one row of pixels in the image.
 * 
 * A thread pool is created at the beginning of the program, with one thread for
 * each available processor. The threads remove tasks from a blocking queue and
 * execute them. The threads never terminate (until the program ends). To start
 * a computation, tasks are created and added to the blocking queue. As soon as
 * the first tasks are added to the queue, the threads "wake up" and start
 * working on them.
 * 
 * (The image is a small piece of the famous Mandelbrot set, which is used just
 * because it takes some time to compute. There is no need to understand what
 * the image means.)
 */
public class MultiprocessingDemo3 extends Application {
	...
	/**
	 * Set up the GUI and event handling. The canvas will be 1200-by-1000 pixels, if
	 * that fits comfortably on the screen; otherwise, size will be reduced to fit.
	 * This method also makes the color palette, containing colors in spectral
	 * order.
	 */
	...
	/**
	 * This method is called from the computation threads when one row of pixels
	 * needs to be added to the image.
	 * 
	...
	/**
	 * This is called the first time the user clicks "Start" to create the thread
	 * pool and the blocking queue of tasks that is used to send tasks to the thread
	 * pool. The number of threads is equal to the number of available processors.
	 * There is no need to keep any references to the thread objects. They only have
	 * to be started here, and they will just keep running until the program ends.
	 */
	...
	/**
	 * This method is called when the user clicks the Start button. If no
	 * computation is in progress, it clears the image and sets up the computation
	 * of a new image. The first time that it is called, it is also responsible for
	 * creating the the thread pool.
	 */
	...
	/**
	 * This method is called by each thread when it terminates. We keep track of the
	 * number of threads that have terminated, so that when they have all finished,
	 * we can put the program into the correct state, such as changing the name of
	 * the button to "Start Again" and re-enabling the pop-up menu. This method is
	 * responsible for drawing the row of pixels computed by the task to the canvas.
	 * It only does that if the task is part of the current job, not a previous,
	 * aborted job.
	 */
	...
	/**
	 * An object of type MandelbrotTask represents the task of computing one row of
	 * pixels in an image of the Mandelbrot set. The task has a run() method that
	 * does the actual computation. It also calls the taskFinished() method before
	 * terminating. It does not draw the row of pixels to the canvas, because it is
	 * possible that the task completes after a job has been aborted. In that case,
	 ...
	 * WorkerThread runs in a loop in which it retrieves a task from the taskQueue
	 * and calls the run() method in that task. Note that if the queue is empty, the
	 * thread blocks until a task becomes available in the queue. The thread will
	 * run at a priority that is one less than the priority of the thread that calls
	 * the constructor.
	 * 
	 * A WorkerThread is designed to run in an infinite loop. It will end only when
	 * the Java virtual machine exits. (This assumes that the tasks that are
	 * executed don't throw exceptions, which is true in this program.) The
	 * constructor sets the thread to run as a daemon thread; the Java virtual
	 * machine will exit when the only threads are daemon threads.
	 */
