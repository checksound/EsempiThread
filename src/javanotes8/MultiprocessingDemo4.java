 ...
 * 
 * The functionality of this program is identical to MultiprocessingDemo3. This
 * version of the program uses an ExecutorService to execute the tasks. When the
 * user clicks "Start", a new ExecutorService is created and all of the tasks
 * that are part of computing the image are added to it. If the user aborts a
 * computation, the executor's shutDownNow() method is called, which makes the
 * executor drop any waiting tasks from its queue.
 * 
...
public class MultiprocessingDemo4 extends Application {
	...
	synchronized private void doStartOrStop() {
		if (executor != null) { // a job is in progress
			...
			for (int row = 0; row < height; row++) { // Add tasks for current job to job queue.
				...
				MandelbrotTask task = new MandelbrotTask(executor, row, width, maxIterations, xmin, y, dx);
	...
	private class MandelbrotTask implements Runnable {
		...
		MandelbrotTask(ExecutorService executor, int rowNumber, int width, int maxIterations, double xmin, double y,
				double dx) {
