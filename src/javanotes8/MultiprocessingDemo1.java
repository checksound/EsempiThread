...
/**
 * This demo program uses several threads to compute an image "in the
 * background". It is a modification of the BackgroundComputationDemo, which
 ...
 * the image means.) The user starts the computation by clicking a "Start"
 * button. A pop-up menu allows the user to select the number of threads to be
 * used. The specified number of threads is created and each thread is assigned
 * a region in the image. The threads are run at lower priority, which will make
 * sure that the GUI thread will get a chance to run to repaint the display as
 * necessary.
 */
public class MultiprocessingDemo1 extends Application {
	 ...
	 * currently running, it starts as many new threads as the user has specified,
	 * and assigns a different part of the image to each thread. The threads are run
	 * at lower priority than the event-handling thread, in order to keep the GUI
	 * responsive. If a computation is in progress when this method is called,
	...
	/**
	 * This class defines the thread that does the computation. The run method
	 * computes the image one pixel at a time. After computing the colors for each
	 * row of pixels, the colors are copied into the image, and the part of the
	 * display that shows that row is repainted. All modifications to the GUI are
	 * made using Platform.runLater(). (Since the thread runs in the background, at
	 * lower priority than the event-handling thread, the event-handling thread
	 * wakes up immediately to repaint the display.)
	 */
