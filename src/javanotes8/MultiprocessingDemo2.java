 ...
 * computes one row of pixels in the image. The tasks are placed into a
 * thread-safe queue. Several "worker" threads remove tasks from the queue and
 * carry them out. When all the tasks have completed, the worker threads
 * terminate. The number of worker threads is specified by the user. (The image
 * is a small piece of the famous Mandelbrot set, which is used just because it
 * takes some time to compute. There is no need to understand what the image
 * means.)
 */
public class MultiprocessingDemo2 extends Application {
	...
	/**
	 * This method is called when the user clicks the button, If no computation is
	 * in progress, then it starts as many new threads as the user has specified. It
	 * creates one MandelbrotTask object for each row of the image and places all
	 * the tasks into a queue. The threads will remove tasks from the queue to
	 * process them. The threads are run at lower priority than the event-handling
	 * thread, in order to keep the GUI responsive. If this method is called when a
	 * computation is in progress, it sets the value of the signal variable, running
	 * to false, as a signal to the threads that they should terminate.
	 */
	 ...
	 * the button to "Start Again" and re-enabling the pop-up menu.
	 */
	 ...
	 * does the actual computation and also applies the colors that it has computed
	 ...
	 * and calls the run() method in that task. The thread terminates when the queue
	 * is empty. (Note that for this to work properly, all the tasks must be placed
	 * into the queue before the thread is started. If the queue is empty when the
	 * thread starts, the thread will simply exit immediately.) The thread also
	 * terminates if the signal variable, running, is set to false. Just before it
