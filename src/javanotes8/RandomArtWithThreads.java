...
/**
 * A program that shows randomly generated "art". When the user clicks a "Start"
 * button, a new random artwork is generated every two seconds.
 * 
 * This program demonstrates using a thread for a very simple animation. The
 * thread uses Platform.runLater() to redraw the canvas on the JavaFX
 * application thread.
 */
...
public class RandomArtWithThreads extends Application {
	...
	/**
	 * If called when an animation is running, this method fills the canvas with a
	 * random work of "art". If no thread is running, it just fills the canvas with
	 * white.
	 */
	private void redraw() {
	 ...
	 * running, it sets the signaling variable, running, to true and starts a new
	 * thread; it also changes the text on the Start button to "Stop". If the user
	 * clicks the button while a thread is running, then a signal is sent to the
	 * thread to terminate, by setting the value of the signaling variable, running,
	 * to false; and the method also sets the text on the Start button back to
	 * "Start."
	 */
	private void doStartOrStop() {
		if (running == false) { // start a thread
			...
			/*
			 * Wait for the thread to stop before setting runner = null. One second should
			 * be plenty of time for this to happen, but in case something goes wrong, it's
			 * better not to wait forever.
			 */
